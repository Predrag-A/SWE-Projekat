<template>
    <div>
        ...
        <div v-if="court_id != -1 &&  this.selectedRating != 0" style="padding-top:10px;cursor:pointer;margin-bottom:20px;color: blue;"><a @click="deleteRating">Resetujte ocenu</a></div>   
    </div>
</template>
...
export default {
    ...
    created() {
        ...
        this.data.court_id = this.court_id
    },
    methods: {
        setRating($event, persist) {
            if (!this.readOnly) {
                ...
                if (persist) {
                    ...
                    this.data.grade = this.selectedRating;
                    if(this.data.court_id != -1){
                        this.saveRating();
                    }
                } else {
        ...
        // Slanje post zahteva u bazu za snimanje
        saveRating(){                        
            const t = this;      
            axios.post('/api/oceniteren', t.data);
        },
        // Slanje post zahteva u bazu za brisanje
        deleteRating(){            
            const t = this;
            axios.post('/api/resetujteren', t.data).then(({data}) => {
                if(data == 1){                    
                    M.toast({html:'Ocena resetovana', classes:'green lighten-3'});
